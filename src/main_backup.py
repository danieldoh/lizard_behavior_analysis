...
from image_processing import undistorted_video
...
if __name__ == "__main__":
    ...
    parser.add_argument("--undistort", "-u", action="store_true", help="undistort?")
    ...
    if (args.undistort):
        ...
        undistorted_video(args.video_name)
    ...
    if (args.pred):
        ...
        video_list = os.listdir(args.video_date)

        print(f"Videos to predict: {video_list}")
        print(f"Number of Videos: {len(video_list)}")

        for video_name in video_list:
            undistorted_video(video_name)
            IMG_DIR = extract_frame(video_name)
            #IMG_DIR = "./prediction/3_8/frames/"
            csv_dict = prepare_predictions(IMG_DIR, video_name, model_pose, COLORS_RGB_MAP)
            data_to_csv(csv_dict, video_date_data_path, video_name)'''

        '''prepare_predictions(args.video_name, model_pose)

        csv_dict = data_denorm(f"../runs/pose/{args.pred_num}/labels/")
        data_to_csv(csv_dict)
        ...
        #for idx, (file, axis) in enumerate(zip(TEST_IMAGE_FILES[:num_samples], ax.flat)):
        #IMG_DIR = "./prediction/11_09/frames/"
        ...
        #plt.savefig(f'./prediction/{formatted_time}.png')'''
